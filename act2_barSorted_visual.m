...
M=length(EEGData);N=ep;Fs = 256;Ts = 1/Fs; % ms -> f=256[Hz]
t = double((0:M-1)*Ts);             % Time vector
segments_vector=1:N; prom_sorted_visual=zeros(M,1);
prom_weigthed_visual=zeros(M,1);prom_standar_visual=zeros(M,1);

% cargada de registros e inicialización de variables
for i=1:N
    rms_visual(i,1)=rms(EEGSegments(:,i));
end
%Normalización de los RMS
for i=1:N
    rms_normalizada(i,1)=(rms_visual(i,1))./mean(rms_visual(:,1));
end

[valor_posicion,posicion]=sort(rms_visual);%ordenamiento de RMS

%Suma de cada metodo de ponderación
for i=1:N
     prom_sorted_visual(:,1)= prom_sorted_visual(:,1) + EEGSegments(:,posicion(i));
     prom_weigthed_visual(:,1)= prom_weigthed_visual(:,1) + (EEGSegments(:,i)/rms_normalizada(i,1));
     prom_standar_visual(:,1)= prom_standar_visual(:,1) + EEGSegments(:,i);  
end

% inicialización de variables para selección de señal según método:
%          --- promedio por pesos ---             %
[max_signal_weigthed,i_max_signal_weigthed] = max(prom_weigthed_visual); % para obtener el Vpp
[min_signal_weigthed,i_min_signal_weigthed] = min(prom_weigthed_visual); % para obtener el Vpp
Vpp_weigthed=max_signal_weigthed - min_signal_weigthed;
largo_ventana_weigthed =abs(i_max_signal_weigthed-i_min_signal_weigthed);
%          --- promedio clasico ---               %
[max_signal_standar,i_max_signal_standar] = max(prom_standar_visual); % para obtener el Vpp
[min_signal_standar,i_min_signal_standar] = min(prom_standar_visual); % para obtener el Vpp
Vpp_standar=max_signal_standar - min_signal_standar;
largo_ventana_standar =abs(i_max_signal_standar-i_min_signal_standar);
%          --- promedio ordena ---                %
[max_signal_sorted,i_max_signal_sorted] = max(prom_sorted_visual); % para obtener el Vpp
[min_signal_sorted,i_min_signal_sorted] = min(prom_sorted_visual); % para obtener el Vpp
...
prom_sorted_visual=prom_sorted_visual/N;  prom_weigthed_visual= prom_weigthed_visual/N;
prom_standar_visual=prom_standar_visual/N;

run('act2_figure_visual_barsorted.m')
run('act2_figure_visual_timefreq.m')
